...
func (s *HolidayService) Create(in struct{ Name, Date, Description, Type, Status string }, actorID, ip, ua string) (*models.Holiday, error) {
...
func (s *HolidayService) Update(id string, in struct{ Name, Date, Description, Type, Status string }, actorID, ip, ua string) (*models.Holiday, error) {
	if _, err := s.holidays.FindByID(id); err != nil {
	...
	if err := s.holidays.Update(id, fields); err != nil {
		return nil, err
	}
	s.audit.Record(actorID, models.ActionUpdate, "holiday", id, ip, ua, nil)
	return s.holidays.FindByID(id)
}

func (s *HolidayService) Get(id string) (*models.Holiday, error) {
	h, err := s.holidays.FindByID(id)
	if err != nil {
		return nil, ErrNotFound
	}
	return h, nil
...
func (s *HolidayService) Delete(id, actorID, ip, ua string) error {
	if _, err := s.holidays.FindByID(id); err != nil {
		return ErrNotFound
	}
	if err := s.holidays.Delete(id); err != nil {
		return err
	}
...
func (s *HolidayService) List(from, to, status string) ([]models.Holiday, error) {
	...
	return s.holidays.List(df, dt, status)
}
